...
void compiler::x86::emitter::assemble(const ir::Binary &binary) {
    ...
    switch (binary.op) {
        ...
        case Greater:
            ...
            current_function_.emplace_back(SetCC(CC::Greater, result));
            break;
        case GreaterEqual:
            current_function_.emplace_back(Cmp(left, right));
            current_function_.emplace_back(Mov(result, Imm{0}));
            current_function_.emplace_back(SetCC(CC::GreaterOrEqual, result));
            break;
        case Less:
            current_function_.emplace_back(Cmp(left, right));
            current_function_.emplace_back(Mov(result, Imm{0}));
            current_function_.emplace_back(SetCC(CC::Less, result));
            break;
        case LessEqual:
            current_function_.emplace_back(Cmp(left, right));
            current_function_.emplace_back(Mov(result, Imm{0}));
            current_function_.emplace_back(SetCC(CC::LessOrEqual, result));
            break;
        case EqualEqual:
            current_function_.emplace_back(Cmp(left, right));
            current_function_.emplace_back(Mov(result, Imm{0}));
        ...
        case NotEqual:
            current_function_.emplace_back(Cmp(left, right));
            current_function_.emplace_back(Mov(result, Imm{0}));
            current_function_.emplace_back(SetCC(CC::NotEqual, result));
            break;
