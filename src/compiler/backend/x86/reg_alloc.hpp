...
namespace compiler::x86 {
    ...
    public:
        ...
        static InterferenceGraph get_graph(const CFG<Instruction>& cfg, const LivenessAnalysis<Instruction>& liveness) {
        ...
        [[nodiscard]] std::string max_degree_node() const {
            return *std::ranges::max_element(nodes_, std::less{}, [this](const auto& node) { return degree(node); });
        }
